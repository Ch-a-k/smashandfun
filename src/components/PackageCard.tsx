  ...
  return (
    <div>
      ...
      <ul className="space-y-2">
        {items.map((item, i) => (
          <motion.li key={i} {...itemAnimation(i)} className="flex items-start gap-2">
            <CheckCircle2
              className={cn('w-4 h-4 mt-1', isBestseller ? 'text-[#f36e21]' : 'text-white/40')}
            />
...
export function PackageCard({
  ...
  const bookingHref = `/booking/${pkg.id}${bookingQuery ? `?${bookingQuery}` : ''}`;
  return (
    <div
      className={cn(
      ...
      >
        <div className="flex justify-between items-start">
          <div>
            ...
            <div className="mt-1 text-xl font-bold text-white">{pkg.price}</div>
          </div>
          {isBestseller && (
